...
export const OrderCategoriesPage = () => {
  const { data, isLoading, isError } = useGetOrderCategoriesQuery();
  ...
  const [createMain, createMainState] = useCreateOrderMainCategoryMutation();
  const [updateMain, updateMainState] = useUpdateOrderMainCategoryMutation();
  const [deleteMain, deleteMainState] = useDeleteOrderMainCategoryMutation();
  const [createAdditional, createAdditionalState] =
    useCreateOrderAdditionalCategoryMutation();
  const [updateAdditional, updateAdditionalState] =
    useUpdateOrderAdditionalCategoryMutation();
  const [deleteAdditional, deleteAdditionalState] =
    useDeleteOrderAdditionalCategoryMutation();
  ...
  const handleDeleteMain = async (id: string, name: string) => {
    if (!window.confirm(`Удалить основную категорию «${name}»?`)) return;
    setActionError(undefined);
    try {
  ...
  const handleDeleteAdditional = async (id: string, name: string) => {
    if (!window.confirm(`Удалить дополнительную категорию «${name}»?`)) return;
    setActionError(undefined);
    try {
  ...
  return (
    <Stack spacing={4}>
      <Typography component="h1" variant="h4">Категории заказов</Typography>
      ...
      <Paper component="section" variant="outlined" sx={{ p: 3 }}>
        <Stack spacing={3}>
          <Typography component="h2" variant="h5">Основные категории</Typography>
          <Stack
            component="form"
      ...
      <Paper component="section" variant="outlined" sx={{ p: 3 }}>
        <Stack spacing={3}>
          <Typography component="h2" variant="h5">Дополнительные категории</Typography>
